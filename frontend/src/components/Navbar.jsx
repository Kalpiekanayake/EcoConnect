...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-[1000] transition-all duration-500 ${scrolled ? 'py-3' : 'py-6'}`}>
      <div className="max-w-7xl mx-auto px-6">
        <div className={`px-6 py-3 rounded-2xl border transition-all duration-500 flex items-center justify-between ${
          scrolled 
            ? 'bg-[#0a1223]/80 backdrop-blur-xl border-white/10 shadow-2xl' 
            : 'bg-[#0a1223]/40 backdrop-blur-md border-white/5 shadow-lg'
        }`}>
          <Link to="/" className="flex items-center gap-2 group">
            ...
            <span className="text-2xl font-black tracking-tighter text-white">EcoConnect</span>
          </Link>
            ...
            {token ? (
              <div className="flex items-center gap-4 pl-6 border-l border-white/10">
                <div className="text-right">
                  <p className="text-[10px] font-black uppercase tracking-widest leading-none mb-1 text-primary">{user?.role}</p>
                  <p className="text-xs font-bold leading-none text-white">{user?.full_name?.split(' ')[0]}</p>
                </div>
                <div className="h-9 w-9 rounded-full flex items-center justify-center border bg-white/10 border-white/20 text-white">
              ...
              <div className="flex items-center gap-8">
                <Link to="/login" className="text-xs font-black uppercase tracking-[0.2em] text-white/80 hover:text-white transition-colors">Login</Link>
                <Link to="/register" className="py-3 px-8 text-xs font-black rounded-xl uppercase tracking-widest bg-white text-primary hover:bg-slate-100 transition-all shadow-xl shadow-primary/10">Join Now</Link>
              </div>
            )}
          </div>
